...
class TituloUniversitarioController extends Zend_Controller_Action
{
    ...
    public function ingresarTituloAction()
    {
        ...
        if($this->_request->getPost())
        {
            ...
            $tituloUniversitario = $this->getRequest()->getParam('titulo_universitario');
            $universidadInstitucion = $this->getRequest()->getParam('universidad_institucion');
            $paisUniversidad = $this->getRequest()->getParam('pais_universidad');
            $ciudadUniversidad = $this->getRequest()->getParam('ciudad_universidad');
            $registroConesup = $this->getRequest()->getParam('registro_conesup');
            $fechaGraduacion = $this->getRequest()->getParam('fecha_graduacion');
            $verificarTitulo = $this->getRequest()->getParam('verificar_titulo');

            $titulo = new Application_Model_DbTable_TituloUniversitario();
            $titulo->agregar_titulo('', $cedulaIdentidad, $tituloUniversitario, $universidadInstitucion,
                    $paisUniversidad, $ciudadUniversidad, $registroConesup, $fechaGraduacion, $verificarTitulo);            
        }
        $table = new Application_Model_DbTable_TituloUniversitario();
        $this->view->cedula = $defaultNamespace->cedula_identidad;
        $cedulaIdentidad =  $defaultNamespace->cedula_identidad;
        $this->view->titulo = $table->listar_titulo_cedula($cedulaIdentidad);
